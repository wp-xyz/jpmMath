...
Function CombiPolFract(F1,F2:FRACTION;a,b:NUMBER;VAR F:FRACTION): Boolean;
Var P:POLYNOM;
Begin
  CombiPolFract:=FALSE;
  if Not GCDPolynom(F1.denom,F2.denom,P) then exit;
  if Not MultPolynom(F1.denom,F2.denom,F.denom) then exit;
  if Not MultPolynom(F1.numer,F2.denom,F1.numer) then exit;
  if Not MultPolynom(F1.denom,F2.numer,F1.denom) then exit;
  if Not CombiPolynom(F1.numer,F1.denom,a,b,F.numer) then exit;
  if Not DivPolynom(F.numer,P,F.numer,F1.numer) then exit;
  if Not DivPolynom(F.denom,P,F.denom,F1.numer) then exit;
...
BEGIN
  ...
  Writeln;
  Writeln(' LINEAR COMBINATION OF TWO POLYNOMIAL FRACTIONS');
  Writeln('            a F1(x) + b F2(x)');
  Writeln;
