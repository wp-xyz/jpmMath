...
Var
  ...
  **************************************************************}
  ...
  {Graph of Fourier analysis of input signal on request (0 to fmax/2) }
  Procedure View_Fourier_Analysis;
  Var fmax:REAL_AR;
  Begin
    ClrScr;
    fmax:=1.0/(4.0*dt);
    CourbeXY(CrtDC,(n1 DIV 2),10,Signal,0,fmax);
    Legendes(CrtDC,'FOURIER ANALYSIS','Freq. (Hz)','Acc. m/s2');
    SortieGraphique
  End;
...
BEGIN
  ...
  WinCrtInit('Fast Fourier Transform');
  ...
  df:=1.0/(ndata*dt);
  {print frequency spectrum to output file}
  f:=0.0;
  ...
  close(f_out); Clrscr;
  writeln;
  writeln(' Fourier analysis done. Results in file tfft.lst.');
  write(' Do you want to view the Fourier analysis (y/n) ? ');
  if readkey='y' then View_Fourier_Analysis;
