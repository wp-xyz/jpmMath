...
INTERFACE
  ...
  USES WinCrtMy,WinProcs, WinTypes, Strings, Type_def;
...
IMPLEMENTATION
  ...
  BEGIN
    ...
    FOR I:=0 TO 300 DO
      FOR J:=0 TO 200 DO
        IF (Pt[0]^[I][J]=1) THEN SetPixel(P,I,J,Bleu);
    l:=301;
    IF MaxX < 600 THEN finx:=MaxX ELSE finx:=600;
    FOR I:=l TO finx DO
      FOR J:=0 TO 200 DO
        IF (Pt[1]^[I-l][J]=1) THEN SetPixel(P,I,J,Bleu);
    m:=201;
    IF MaxY < 445 THEN finy:=MaxY-45 ELSE finy:=400;
    FOR I:=0 TO 300 DO
      FOR J:=m TO finy DO
        IF (Pt[2]^[I][J-m]=1) THEN SetPixel(P,I,J,Bleu);
    FOR I:=l TO finx DO
      FOR J:=m TO finy DO
        IF (Pt[3]^[I-l][J-m]=1) THEN SetPixel(P,I,J,Bleu);
    for i:=0 to 3 do Dispose(Pt[i])
  END;
